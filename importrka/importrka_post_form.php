...
function importrka_post_form(){
	...
	$sql = 'select kegiatan,tahun from {kegiatanskpd} where kodekeg=\'%s\'';
	$res = db_query(db_rewrite_sql($sql), array ($kodekegasal));
	if ($res) {
		$data = db_fetch_object($res);
		if ($data) {
			$kegiatanasal = $data->kegiatan . ' (' . $data->tahun . ')';
	...
	$form['kodekegasal']= array(
		'#type'         => 'hidden', 
		'#default_value'=> $kodekegasal, 
	); 

	$form['kegiatan']= array(
		'#type'         => 'markup', 
		'#value'		=> '<p>Import RKA-SKPD untuk Kegiatan <strong style="font-size:15px">' . $kegiatan . '</strong> dari Kegiatan <strong style="font-size:15px">' . $kegiatanasal . '</strong>.</p>', 
	); 
	if ($total>0) {
	...
	$form['kegiatanrek']= array(
		'#type'         => 'markup', 
		'#value'		=> GenReportFormContent($kodekegasal), 
	); 
...
function importrka_post_form_submit($form, &$form_state) {

	
	$kodekeg = $form_state['values']['kodekeg'];
	$kodekegasal = $form_state['values']['kodekegasal'];
	...
	//Anggperkeg
	$total = 0;
	$sql = 'select r.kodero,r.uraian,k.jumlah from {anggperkeg} k inner join {rincianobyek} r on k.kodero=r.kodero where k.kodekeg=\'%s\'';
	$fsql = sprintf($sql, db_escape_string($kodekegasal));
	...
	//anggperkegdetil
	$sql = 'select k.kodekeg, k.kodero, k.nourut, k.iddetil, k.uraian, k.unitjumlah, k.unitsatuan, k.volumjumlah, k.volumsatuan, k.harga, k.total, k.pengelompokan from {anggperkegdetil} k inner join {rincianobyek} r on k.kodero=r.kodero where k.total>0 and k.kodekeg=\'%s\' order by k.kodero';
	$fsql = sprintf($sql, db_escape_string($kodekegasal));	
	//drupal_set_message($fsql);
	$result = db_query($fsql);
	if ($result) {
		while ($data = db_fetch_object($result)) {
			...
			if ($data->pengelompokan) {
				//SUB DETIL
				$sql = 'select idsub,nourut,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan, harga,total, anggaran from {anggperkegdetilsub} where total>0 and iddetil=\'%s\' order by nourut asc,idsub';
		...
		while ($datajenis = db_fetch_object($resultjenis)) {
			...
			if ($resultobyek) {
				while ($dataobyek = db_fetch_object($resultobyek)) {
					...
					//REKENING
					$sql = 'select kodero,uraian,jumlah from {anggperkeg} k where jumlah>0 and kodekeg=\'%s\' and mid(k.kodero,1,5)=\'%s\'';
					...
					if ($result) {
						while ($data = db_fetch_object($result)) {
							...
							//DETIL
							$sql = 'select iddetil,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan,harga,total,anggaran, pengelompokan from {anggperkegdetil} where total>0 and kodekeg=\'%s\' and kodero=\'%s\' order by nourut asc,iddetil';
