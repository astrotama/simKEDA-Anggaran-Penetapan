							...
							if ($tingkat>=4) {
								...
								if ($resulto) {
									while ($datao = db_fetch_object($resulto)) {
										...
										if ($tingkat>=5) {
											if ($kodeuk!='00') {
												$sql = 'select r.kodero,r.uraian,sum(k.jumlah) jumlahx ,sum(jumlahp) jumlahxp from {anggperuk} k inner join {rincianobyek} r on k.kodero=r.kodero where kodeuk=\'%s\' and mid(k.kodero,1,5)=\'%s\' and k.jumlah>0';
												$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datao->kodeo));
												$fsql .= ' order by r.kodero';
												
											} else {
												$sql = 'select r.kodero,r.uraian,sum(k.jumlah) jumlahx ,sum(jumlahp) jumlahxp from {anggperuk} k inner join {rincianobyek} r on k.kodero=r.kodero where mid(k.kodero,1,5)=\'%s\' and k.jumlah>0';
