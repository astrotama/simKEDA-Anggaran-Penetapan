...
function GenReportFormPerubahan($kodeuk, $tingkat, $tipedok) {
	...
	if ($tipedok=='dpa') {
		$rowsjudul[] = array (array ('data'=>'RINGKASAN DOKUMEN PELAKSANAAN PERUBAHAN ANGGARAN', 'width'=>'875', 'colspan'=>'3', 'style' =>'border:none; font-weight:900; font-size:1.3em; text-align:center;'));
	
	} else {
		$rowsjudul[] = array (array ('data'=>'RINGKASAN PERUBAHAN ANGGARAN PENDAPATAN DAN BELANJA DAERAH', 'width'=>'875', 'colspan'=>'3', 'style' =>'border:none; font-weight:900; font-size:1.3em; text-align:center;'));
	}
...
function GenReportFormFooter($kodeuk, $tipedok) {
	...
	if ($tipedok=='dpa') {
		...
		$pquery = sprintf("select dpatgl1, setdanama, setdanip, setdajabatan from {setupapp} where tahun='%s'", variable_get('apbdtahun', 0)) ;
		////drupal_set_message($pquery);
		$pres = db_query($pquery);
		if ($data = db_fetch_object($pres)) {
			$setdanama = $data->setdanama;
