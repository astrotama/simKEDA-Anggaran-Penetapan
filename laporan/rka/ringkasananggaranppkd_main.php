...
function GenReportForm($print=0,$revisi) {
	...
	$sql = "select a.kodea,a.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k  inner join {anggaran} a on mid(k.kodero,1,1)=a.kodea inner join {kegiatanperubahan' . $str_table . '} g on k.kodekeg=g.kodekeg where g.inaktif=0 and mid(k.kodero,1,3)>'511' ";
	...
	if ($resultakun) {
		while ($dataakun = db_fetch_object($resultakun)) {
			...
			//KELOMPOK
			$sql = 'select x.kodek,x.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k inner join {kelompok} x on mid(k.kodero,1,2)=x.kodek inner join {kegiatanperubahan' . $str_table . '} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.isppkd=1 and mid(k.kodero,1,1)=\'%s\' and mid(k.kodero,1,3)>\'%s\'';
			$fsql = sprintf($sql, db_escape_string($dataakun->kodea), '511');
			$fsql .= ' group by x.kodek,x.uraian order by x.kodek';
			...
			if ($resultkel) {
				while ($datakel = db_fetch_object($resultkel)) {
					...
					//JENIS
					$sql = 'select j.kodej,j.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej  inner join {kegiatanperubahan' . $str_table . '} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.isppkd=1 and mid(k.kodero,1,2)=\'%s\'';
					...
					if ($result) {
						while ($data = db_fetch_object($result)) {
							...
							//OBYEK
							if ($tingkat>=4) {
								$sql = 'select r.kodeo,r.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k inner join {obyek} r on mid(k.kodero,1,5)=r.kodeo inner join {kegiatanperubahan' . $str_table . '} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.isppkd=1 and mid(k.kodero,1,3)=\'%s\'';
								...
								if ($resulto) {
									while ($datao = db_fetch_object($resulto)) {
										...
										//RINCIAN OBYEK 
										if ($tingkat>=5) {
											$sql = 'select r.kodero,r.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k inner join {rincianobyek} r on k.kodero=r.kodero inner join {kegiatanperubahan' . $str_table . '} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.isppkd=1 and mid(k.kodero,1,5)=\'%s\'';
...
function GenReportFormContent($kodeuk, $tingkat, $revisi) {
	...
	//****BELANJA
	$totalb = 0;$totalbp = 0;$totalbt = 0;
	$sql = 'select a.kodea,a.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k  inner join {anggaran} a on mid(k.kodero,1,1)=a.kodea inner join {kegiatanperubahan' . $str_table . '} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.isppkd=1 ';
	$fsql = sprintf($sql, db_escape_string($kodeuk));
	$fsql .= ' group by a.kodea,a.uraian order by a.kodea';
	...
	if ($resultakun) {
		while ($dataakun = db_fetch_object($resultakun)) {
			...
			//KELOMPOK
			$sql = 'select x.kodek,x.uraian,sum(jumlah) jumlahx,sum(jumlahp) jumlahxp from {anggperkegperubahan' . $str_table . '} k inner join {kelompok} x on mid(k.kodero,1,2)=x.kodek inner join {kegiatanperubahan' . $str_table . '} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.isppkd=1 and mid(k.kodero,1,1)=\'%s\'';
...
function GenReportFormFooter() {
	...
	if ($tipedok=='dpa') {
		...
		*/
		...
		$tw3 = 0;
		$tw4 = 0;
		$pquery = 'select sum(tw1) tw1t,sum(tw2) tw2t,sum(tw3) tw3t,sum(tw4) tw4t from {kegiatanperubahan} where jenis=1 and inaktif=0 and isppkd=1';
		//drupal_set_message($pquery);
		$pres = db_query($pquery);
		if ($data = db_fetch_object($pres)) {
			$tw1 = $data->tw1t;
