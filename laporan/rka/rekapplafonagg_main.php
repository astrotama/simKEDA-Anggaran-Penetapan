...
function GenReportForm($kodeuk, $jenis, $detil,$revisi) {
	...
	if ($result) {
		while ($data = db_fetch_object($result)) {
			...
			if ($jenis=='') {
				$sql = 'select sum(a.jumlah) tanggaran from {kegiatanskpd} k  inner join {anggperkeg} a on k.kodekeg=a.kodekeg inner join {rincianobyek} ro on a.kodero=ro.kodero where k.inaktif=0 and k.kodeuk=\'%s\' ';
				$fsql = sprintf($sql, db_escape_string($data->kodeuk));
			} else {
				if ($jenis=='1') {
					$sql = 'select  sum(a.jumlah) tanggaran from {kegiatanskpd} k  inner join {anggperkeg} a on k.kodekeg=a.kodekeg inner join {rincianobyek} ro on a.kodero=ro.kodero where k.inaktif=0 and k.kodeuk=\'%s\' and k.jenis=\'%s\' and left(a.kodero,2)=\'%s\'';
					$fsql = sprintf($sql, db_escape_string($data->kodeuk), db_escape_string($jenis), '51');
				} else {
					$sql = 'select  sum(a.jumlah) tanggaran from {kegiatanskpd} k  inner join {anggperkeg} a on k.kodekeg=a.kodekeg inner join {rincianobyek} ro on a.kodero=ro.kodero where k.inaktif=0 and k.kodeuk=\'%s\' and k.jenis=\'%s\' and left(a.kodero,3)=\'%s\'';
					$fsql = sprintf($sql, db_escape_string($data->kodeuk), db_escape_string($jenis), '52');
				}
			}			
			$resultpa = db_query($fsql);
			if ($resultpa) {
