...
drupal_add_css('files/css/kegiatancam.css');
	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {
		...
		if($tipedok=='dpaperubahan')
		{
				...
				$htmlFooter = GenReportFormPerubahanFooter($kodeuk, $tipedok);
		...
		else
		{
				...
				$htmlContent = GenReportFormContent($kodeuk, $tipedok,$revisi);
...
function GenReportForm($kodeuk, $tipedok,$revisi) {
		...
		while ($datapro = db_fetch_object($resultpro)) {
			...
			if ($resultkeg) {
				while ($datakeg = db_fetch_object($resultkeg)) {
					...
					$rowsrek[] = array (
											 array('data' => $datapro->kodepro . '.' . $datakeg->nomorkeg, 'width' => '95px', 'style' => 'border-left:1px solid black;text-align:left;font-size: 100%;'),
		...
		while ($datapro = db_fetch_object($resultpro)) {
			...
			if ($resultkeg) {
				while ($datakeg = db_fetch_object($resultkeg)) {
					...
					$rowsrek[] = array (
											array('data' => $datapro->kodepro . '.' . $datakeg->nomorkeg, 'width' => '75px', 'style' => 'border-left:1px solid black;text-align:left;font-size: 100%;'),
...
function GenReportFormPerubahanFooter($kodeuk, $tipedok) {
