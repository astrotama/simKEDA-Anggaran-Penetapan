<?php
function kegiatanskpd_view_main($arg=NULL, $nama=NULL) {
...
function GetDataView($fsql, $fcountsql, $kodeuk , $sumberdana , $statusisi , $kodesuk , $statustw, $statusinaktif, $jenis, $statusbintang) {
			...
			if ($allowedit) {
				if (isVerifikator()) {
					if ($data->total==0) {
						...
						$editlink =l($rek, 'apbd/kegiatanskpd_view/rekening/' . $data->kodekeg, array('html'=>TRUE));
					}
	...
	$btn = l('Cetak', 'apbd/kegiatanskpd_view/filter/' . $kodeuk . '/' . $sumberdana . '/' . $statusisi . '/' . $kodesuk . '/'. $statustw . '/' . $statusinaktif . '/' . $jenis . '/' . $statusbintang . '/pdf' , array ('html' => true, 'attributes'=> array ('class'=>'btn_blue', 'style'=>'color:white;')));
...
function kegiatanskpd_main_form_submit($form, &$form_state) {
	...
	$uri = 'apbd/kegiatanskpd_view/filter/' . $kodeuk . '/' . $sumberdana . '/' . $statusisi . '/' . $kodesuk . '/'. $statustw . '/' . $statusinaktif . '/' . $jenis . '/' . $statusbintang;
	drupal_goto($uri);
