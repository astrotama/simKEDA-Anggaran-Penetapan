<?php
function rekening_view_main($arg=NULL, $nama=NULL) {
   ...
   if ($arg) {
	...
	$isgaji = false;
	$sql = sprintf('select kegiatan,kodeuk from {kegiatanskpd} where jenis=1 and kodekeg=\'%s\'', $kodekeg);
	//drupal_set_message($sql);
	$res_usr = db_query($sql);
	if ($res_usr) {
		if ($data_user = db_fetch_object($res_usr)) {
			$x = strpos(strtoupper($data_user->kegiatan), 'GAJI');
			
			$kodeuk = $data_user->kodeuk;
			if ($x >=0) $isgaji = true;
		}
	}
	...
	if ($isgaji) {
		...
		$header = array (
			...
			array('data' => 'Plafon', 'valign'=>'top', 'width'=>'90px'),
    ...
    if ($result) {
        while ($data = db_fetch_object($result)) {
			...
			if ($isgaji==true) {
				...
				$plafon = $data->jumlah;
				$sql = sprintf('select plafon from {anggperkegplafon} where kodekeg=\'%s\' and kodero=\'%s\'', $kodekeg, $data->kodero);
				$res_usr = db_query($sql);
				if ($res_usr) {
					if ($data_user = db_fetch_object($res_usr)) {
						$plafon = $data_user->plafon;
					}
				}
				if ($plafon < $data->jumlah) {
					$catatan .= '<font color="Red">Anggaran melebihi plafon, harap diperbaiki</font>';			
					$plafon_s = '<font color="Red">' . apbd_fn($plafon) . '</font>';
					
				} else {
					$plafon_s = apbd_fn($plafon);
				}
				...
				$rows[] = array (
					...
					array('data' => $plafon_s, 'align' => 'right', 'valign'=>'top'),
	...
	//BINTANG
	$jumlah_bintang = 0;
	$pquery = sprintf("select sum(jumlah) totalx  from {anggperkeg} where bintang=1 and kodekeg='%s'", db_escape_string($arg));
	$res = db_query($pquery);
	if ($data_b = db_fetch_object($res)) {
		$jumlah_bintang = $data_b->totalx;
	}	
	if ($jumlah_bintang==0) {
		//BINTANG
		$pquery = sprintf("select sum(total) totalx  from {anggperkegdetil} where bintang=1 and kodekeg='%s'", db_escape_string($arg));
		$res = db_query($pquery);
		if ($data_b = db_fetch_object($res)) {
			$jumlah_bintang = $data_b->totalx;
		}			
	...
	//TUNDA
	$jumlah_tunda = 0;
	$pquery = sprintf("select sum(total) totalx  from {anggperkegdetil} where total<0 and kodekeg='%s'", db_escape_string($arg));
	$res = db_query($pquery);
	if ($data_b = db_fetch_object($res)) {
		$jumlah_tunda = -$data_b->totalx;
	}	
	...
	if ($data = db_fetch_object($pres)) {
		...
		$rows1[] = array (
			array('data' => 'Plafon: ' . apbd_fn($data->plafon) . ', Anggaran: ' . apbd_fn($data->total) . ', Bintang: ' . apbd_fn($jumlah_bintang) . ', Tunda: ' . apbd_fn($jumlah_tunda) . ', Tersedia: ' . apbd_fn($anggaran), 'colspan'=>'6', 'align' => 'right', 'valign'=>'top')
		);	
		...
		if ($jenis=='2') {
			//$output2 .="&nbsp;" .  l('Simpan Excel', 'apbd/kegiatanskpd_view/print/' . $kodekeg . '/excel' , array ('html' => true, 'attributes'=> array ('class'=>'btn_blue', 'style'=>'color:white;'))) ;
			//$output2 .="&nbsp;" .  l('Simpan CSV', 'apbd/kegiatanskpd_view/print/' . $kodekeg . '/csv' , array ('html' => true, 'attributes'=> array ('class'=>'btn_blue', 'style'=>'color:white;'))) ;
		}		
		...
		//Cetak
		$output2 .="&nbsp;" .  l('Preview RKA', 'apbd/kegiatanskpd_view/print/' . db_escape_string($arg) , array ('html' => true, 'attributes'=> array ('class'=>'btn_blue', 'style'=>'color:white;'))) ;
		
		//$output2 .="&nbsp;" .  l('Preview RKA Sebelum Koreksi', 'apbd/kegiatanskpd_pra/print/' . db_escape_string($arg) , array ('html' => true, 'attributes'=> array ('class'=>'btn_red', 'style'=>'color:white;'))) ;
