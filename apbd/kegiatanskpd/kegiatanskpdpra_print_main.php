<?php
function kegiatanskpdpra_print_main() {
	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {
		if (isset($sampul))  {
			if ($sampul=='sampul') {
				...
				apbd_ExportPDF_Sampul('L', 'F4', $htmlContent, $pdfFile);
			} else if ($sampul=='bandingkan_header') {
				$output = drupal_get_form('kegiatanskpdpra_print_form');
				
				$htmlHeader = GenReportFormHeader($kodekeg, $tipedok);
				$htmlHeaderafter = GenReportFormHeaderafter($kodekeg, $tipedok);
				if(arg(8)){
					apbd_ExportPDF2('L', 'F4', $htmlHeader, $htmlHeaderafter, $pdfFile);
				} else {
					//$output .=  '<p><font color="Red">SEBELUM KOREKSI</font><p>' . $htmlHeader . '<p><font color="Blue">SETELAH KOREKSI</font><p>' . $htmlHeaderafter ;
					$output .=  $htmlHeader .  $htmlHeaderafter ;
				}
				//$output .= GenReportForm($kodekeg, $tipedok);
				
				return $output;
			} else if ($sampul=='bandingkan_rekening') {
				$output = drupal_get_form('kegiatanskpdpra_print_form');
				
				$htmlContent = GenReportFormContent($kodekeg);
				$htmlContentafter = GenReportFormContentafter($kodekeg);
				 
				//$output .= GenReportForm($kodekeg, $tipedok);
				if(arg(8)){
					apbd_ExportPDF2('L', 'F4', $htmlContent, $htmlContentafter, $pdfFile);
				} else {
					$output .=  '<h2><font color="Red">SEBELUM DIKOREKSI</font></h2>' . $htmlContent . '<h2><font color="Blue">SETELAH DIKOREKSI</font></h2>' . $htmlContentafter ;
				}
				return $output;
...
function GenReportFormSampulBelanja($kodekeg, $ubahpdf2) {
	...
	$pquery = sprintf('select k.kodekeg,  right(k.kodekeg,3) nomorkeg, k.tahun, k.kodepro, k.kodeuk, k.kodesuk, 
				...
				from {kegiatanskpd_pra} k left join {program} p on (k.kodepro = p.kodepro) 
...
function GenReportFormSampulDepan($kodeuk, $ubahpdf2, $ubahpdf) {
	...
	$rows[]= array (
				array('data' => '',  'width'=> '50px', 'style' => 'border-none;'),
				array('data' => 'KODE',  'width'=> '140px', 'style' => 'border-left: 1px solid black; border-right: 1px solid black; border-bottom: 2px solid black; border-top: 2px solid black; font-weight:900; font-size:1em; text-align:left;'),
...
function GenReportForm($kodekeg, $tipedok) {
	...
	*/
	...
	$pimpinannip='';
	$pimpinanjabatan='';
	$pquery = sprintf("select kodedinas, namauk, pimpinannama, pimpinannip, pimpinanjabatan from {unitkerja} u left join {kegiatanskpd_pra} k 
	...
	$pquery = sprintf('select k.kodekeg, right(k.kodekeg,3) nomorkeg, k.tahun, k.kodepro, k.kodeuk, k.kodesuk, 
				...
				p.kodepro, p.kodeu, u.urusan, u.kodef, u.fungsi, k.tw1, k.tw2, k.tw3, k.tw4 from {kegiatanskpd_pra} k left join {program} p 
...
if ($jenis==2) {
	...
	$total = 0;
	$where = ' where k.kodekeg=\'%s\'';
	$sql = 'select mid(k.kodero,1,3) kodej,j.uraian,sum(jumlah) jumlahx from {anggperkeg_pra} k  left join {jenis} j on mid(k.kodero,1,3)=j.kodej ' . $where;
	$fsql = sprintf($sql, db_escape_string($kodekeg));
	...
	if ($resultjenis) {
		while ($datajenis = db_fetch_object($resultjenis)) {
			...
			//OBYEK
			$sql = 'select mid(k.kodero,1,5) kodeo,o.uraian,sum(jumlah) jumlahx from {anggperkeg_pra} k  left join {obyek} o on mid(k.kodero,1,5)=o.kodeo 
			...
			if ($resultobyek) {
				while ($dataobyek = db_fetch_object($resultobyek)) {
					...
					//REKENING
					$sql = 'select kodero,uraian,jumlah from {anggperkeg_pra} k where kodekeg=\'%s\' and mid(k.kodero,1,5)=\'%s\'';
					...
					if ($result) {
						...
						$rowsrek[] = array (
							...
							//DETIL
							$sql = 'select iddetil, uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan, harga,total,pengelompokan from {anggperkegdetil_pra} where kodekeg=\'%s\' and kodero=\'%s\' order by nourut asc,iddetil';
							...
							if ($resultdetil) {
								while ($datadetil = db_fetch_object($resultdetil)) {
									...
									if ($datadetil->pengelompokan) {
										//SUB DETIL
										$sql = 'select idsub,uraian,unitjumlah,unitsatuan,volumjumlah, volumsatuan,harga,total from {anggperkegdetilsub_pra} where iddetil=\'%s\' order by nourut asc,idsub';
	...
	if ($tipedok=='dpa') {						 
		...
		$pquery = sprintf("select count(kodero) jmlrek from {anggperkeg_pra} where (jumlah mod 1000)>0 and  kodekeg='%s'", db_escape_string($kodekeg));
		////drupal_set_message($pquery); 
		$pres = db_query($pquery);
		if ($data = db_fetch_object($pres)) {
...
function GenReportFormHeader($kodekeg, $tipedok) {
	...
	$pquery = sprintf('select k.kodekeg, right(k.kodekeg,3) nomorkeg, k.tahun, k.kodepro, k.kodeuk, k.kodesuk, k.kegiatan, k.lokasi, k.jenis, 
				...
				p.kodepro, p.kodeu, u.urusan, u.fungsi, u.kodef, uk.kodedinas, uk.namauk from {kegiatanskpd_pra} k left join {program} p on (k.kodepro = p.kodepro) 
				left join {urusan} u on p.kodeu=u.kodeu left join {unitkerja} uk on k.kodeuk=uk.kodeuk ' . $where, db_escape_string($kodekeg));
	////drupal_set_message($pquery);
	$pres = db_query($pquery);
	if ($data = db_fetch_object($pres)) {
	...
	*/
	...
	$rowskegiatan[]= array ( 
							 array('data' => 'RKA-SKPD SEBELUM KOREKSI', 'width' => '875', 'colspan'=>'7',  'style' => 'border-bottom: 1px solid black;border-left: 1px solid black; border-right: 1px solid black; font-size:1em; text-align:center;text-align:center;color: red;'),
							 );		
	$rowskegiatan[]= array (
	...
	*/
	...
	$rowskegiatan[]= array (
							 array('data' => 'Catatan',   'width'=> '150px', 'style' => 'border-left: 1px solid black;  border-top: 1px solid black; text-align:left;'),
							 array('data' => ':',  'width' => '15px', 'style' => ' border-top: 1px solid black; text-align:right;'),
							 array('data' => '<em>'. get_catatan_kegiatan($kodekeg) .'</em>',   'width' => '710', 'colspan'=>'5',  'style' => 'border-right: 1px solid black;  border-top: 1px solid black; text-align:left;'),
	);	
	if ($jenis==2)
		$rowskegiatan[]= array (
							 array('data' => 'Rincian Dokumen Pelaksanaan Anggaran Belanja Langsung per Program dan Kegiatan Satuan Kerja Perangkat Daerah',   'width' => '875', 'colspan'=>'7',  'style' => 'border-left: 1px solid black;  border-right: 1px solid black;  border-top: 1px solid black; text-align:center;border-bottom: 1px solid black;'),
							 );							 
	else
	$rowskegiatan[]= array (
						 array('data' => 'Rincian Dokumen Pelaksanaan Anggaran Belanja Tidak Langsung Satuan Kerja Perangkat Daerah',   'width' => '875', 'colspan'=>'7',  'style' => 'border-left: 1px solid black;  border-right: 1px solid black;  border-top: 1px solid black; text-align:center;border-bottom: 1px solid black;'),
						 );							 
...
function GenReportFormContent($kodekeg) {
	...
	$sql = sprintf('select bintang from {kegiatanskpd_pra} where kodekeg=\'%s\'', db_escape_string($kodekeg));
	////drupal_set_message($pquery);
	$res = db_query($sql);
	if ($res) {
		if ($data = db_fetch_object($res)) {
	...
	$jumrincian = 0;
	$sql = 'select count(iddetil) jumrincian from {anggperkegdetil_pra} where kodekeg=\'%s\'';
	$fsql = sprintf($sql, db_escape_string($kodekeg));
	$res = db_query($fsql);
	if ($res) {
		if ($data = db_fetch_object($res)) {
	...
	$sql = 'select count(idsub) jumrincian from {anggperkegdetilsub_pra} s inner join {anggperkegdetil_pra} d
			on s.iddetil=d.iddetil where d.kodekeg=\'%s\'';
	$fsql = sprintf($sql, db_escape_string($kodekeg));
	$res = db_query($fsql);
	if ($res) {
		...
		while ($datajenis = db_fetch_object($resultjenis)) {
			...
			if ($resultobyek) {
				while ($dataobyek = db_fetch_object($resultobyek)) {
					...
					//REKENING
					$sql = 'select kodero,uraian,jumlah,bintang from {anggperkeg_pra} k where kodekeg=\'%s\' and mid(k.kodero,1,5)=\'%s\'';
					$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($dataobyek->kodeo));
					...
					if ($result) {
						while ($data = db_fetch_object($result)) {
							...
							//DETIL
							if ($tipedok=='rka') {
								$sql = 'select iddetil,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan,harga,total,bintang, pengelompokan from {anggperkegdetil_pra} where total>0 and kodekeg=\'%s\' and kodero=\'%s\' order by nourut asc,iddetil';
								$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($data->kodero));
							} else {	
								...
								if ($rek_bintang) {
									...
									$sql = 'select iddetil,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan,harga,total,bintang, pengelompokan from {anggperkegdetil_pra} where kodekeg=\'%s\' and kodero=\'%s\' order by nourut asc,iddetil';
								}
								$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($data->kodero));
							}
								////drupal_set_message($fsql);
							...
							if ($resultdetil) {
								while ($datadetil = db_fetch_object($resultdetil)) {
									...
									$rowsrek[] = array (
														 array('data' => '',  'width'=> '75px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
														 array('data' => $bullet,  'width' => '20px', 'style' => 'text-align:right;'),
														 array('data' => $datadetil->uraian . $status_detil ,  'width' => '380px', 'style' => ' border-right: 1px solid black; text-align:left;font-weight:lighter;'),
									...
									if ($datadetil->pengelompokan) {
										//SUB DETIL
										$sql = 'select idsub,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan, harga,total, bintang from {anggperkegdetilsub_pra} where iddetil=\'%s\' order by nourut asc,idsub';
										$fsql = sprintf($sql, db_escape_string($datadetil->iddetil));
										////drupal_set_message($fsql);
										...
										if ($resultsub) {
											while ($datasub = db_fetch_object($resultsub)) {
												...
												$rowsrek[] = array (
														 array('data' => '',  'width'=> '75px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
														 array('data' => '',  'width' => '20px', 'style' => 'text-align:right;'),
														 array('data' =>  '- ' . $datasub->uraian . $status_sub,  'width' => '380px', 'style' => ' border-right: 1px solid black; text-align:left;font-weight:lighter;font-style: italic;'),
														 array('data' => $datasub->unitjumlah . ' ' . $datasub->unitsatuan, 'width' => '100px', 'style' => ' border-right: 1px solid black; text-align:center;font-weight:lighter;font-style: italic;'),
														 array('data' => $datasub->volumjumlah . ' ' . $datasub->volumsatuan, 'width' => '100px', 'style' => ' border-right: 1px solid black; text-align:center;font-weight:lighter;font-style: italic;'),
														 array('data' => apbd_fn($datasub->harga),  'width' => '100px', 'style' => ' border-right: 1px solid black; text-align:right;font-weight:lighter;font-style: italic;'),
														 array('data' => apbd_fn($datasub->total),  'width' => '100px', 'style' => ' border-right: 1px solid black; text-align:right;font-weight:lighter;font-style: italic;'),
							...
							if (($tipedok!='dpa') and (variable_get("cetakverifikasirka", 0)==1)) {
								...
								while ($data_cat= db_fetch_object($res_r)) {	
									...
									$rowsrek[] = array (
														 array('data' => '',  'width'=> '75px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
														 array('data' => $bullet,  'width' => '20px', 'style' => 'text-align:center;'),
														 array('data' => $catatan,  'width' => '380px', 'style' => ' border-right: 1px solid black; text-align:left;font-weight:lighter;'),
						...
						$rowsrek[] = array (
														 array('data' => '',  'width'=> '75px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;border-right: 1px solid black;border-bottom: 1px solid black; border-top: 1px solid black;'),
														 array('data' => 'Catatan:','valign' => 'top',  'width' => '50px', 'style' => 'text-align:center;border-bottom: 1px solid black; border-top: 1px solid black;'),
														 array('data' =>  '<em>'. get_catatan_rekening($kodekeg, $data->kodero) .'</em>',  'width' => '350px', 'style' => ' border-right: 1px solid black;border-bottom: 1px solid black; border-top: 1px solid black;text-align:left;font-weight:lighter;'),
														 array('data' => '', 'width' => '100px', 'style' => ' border-right: 1px solid black; text-align:center;font-weight:lighter;border-right: 1px solid black;border-bottom: 1px solid black; border-top: 1px solid black;'),
														 array('data' => '', 'width' => '100px', 'style' => ' border-right: 1px solid black; text-align:center;font-weight:lighter;border-right: 1px solid black;border-bottom: 1px solid black; border-top: 1px solid black;'),
														 array('data' => '',  'width' => '100px', 'style' => ' border-right: 1px solid black; text-align:right;font-weight:lighter;border-right: 1px solid black;border-bottom: 1px solid black; border-top: 1px solid black;'),
														 array('data' => '',  'width' => '100px', 'style' => ' border-right: 1px solid black; text-align:right;font-weight:lighter;border-right: 1px solid black;border-bottom: 1px solid black; border-top: 1px solid black;'),
														 );			
...
function GenReportFormContentBintang($kodekeg) {
	...
	$sql = 'select anggaran from {kegiatanskpd_pra} where kodekeg=\'%s\'';
	$fsql = sprintf($sql, db_escape_string($kodekeg));
	$res = db_query($fsql);
	if ($res) {
		if ($data = db_fetch_object($res)) {
		...
		while ($datajenis = db_fetch_object($resultjenis)) {
			...
			if ($resultobyek) {
				while ($dataobyek = db_fetch_object($resultobyek)) {
					...
					//REKENING
					$sql = 'select kodero,uraian,jumlah,anggaran from {anggperkeg_pra} k where kodekeg=\'%s\' and mid(k.kodero,1,5)=\'%s\'';
					$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($dataobyek->kodeo));
					...
					if ($result) {
						while ($data = db_fetch_object($result)) {
							...
							//DETIL
							$sql = 'select iddetil,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan,harga,total,anggaran, pengelompokan from {anggperkegdetil_pra} where kodekeg=\'%s\' and kodero=\'%s\' order by nourut asc,iddetil';
							$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($data->kodero));
							////drupal_set_message($fsql);
							...
							if ($resultdetil) {
								while ($datadetil = db_fetch_object($resultdetil)) {
									...
									if ($datadetil->pengelompokan) {
										//SUB DETIL
										$sql = 'select idsub,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan, harga,total, anggaran from {anggperkegdetilsub_pra} where iddetil=\'%s\' order by nourut asc,idsub';
										$fsql = sprintf($sql, db_escape_string($datadetil->iddetil));
										////drupal_set_message($fsql);
...
function GenReportFormFooter($kodekeg, $tipedok) {
	...
	if ($tipedok=='dpa') {
		$pquery = sprintf("select tw1, tw2, tw3, tw4, total, anggaran from {kegiatanskpd_pra} where kodekeg='%s'", db_escape_string($kodekeg)) ;
		$pres = db_query($pquery);
		if ($data = db_fetch_object($pres)) {
		...
		$pimpinannip='';
		$pimpinanjabatan='';
		$pquery = sprintf("select u.kodedinas, u.namauk, u.pimpinannama, u.pimpinannip, u.pimpinanjabatan, k.plafon, k.total from {unitkerja} u left join {kegiatanskpd_pra} k 
				  on u.kodeuk=k.kodeuk where k.kodekeg='%s'", db_escape_string($kodekeg)) ;
		$pres = db_query($pquery);
		if ($data = db_fetch_object($pres)) {
...
function GenCSV($kodekeg) {
	...
	$pquery = sprintf('select k.kodekeg, right(k.kodekeg,3) nomorkeg, k.tahun, k.kodepro, k.kodeuk, k.kodesuk, 
				...
				k.totalsebelum, k.totalsesudah, k.waktupelaksanaan, k.sumberdana1, k.sumberdana2, 
				k.sumberdana1rp, k.sumberdana2rp, k.latarbelakang, k.kelompoksasaran, p.program,
				p.kodepro, p.kodeu, u.urusan, u.kodef, u.fungsi, k.tw1, k.tw2, k.tw3, k.tw4, uk.kodedinas, uk.namauk from {kegiatanskpd_pra} k left join {program} p 
...
function kegiatanskpdpra_print_form () {
	$form['formdata'] = array (
		'#type' => 'fieldset',
	...
	if ($tipedok=='dpa') {
		...
		$form['formdata']['bandingkan_header'] = array (
			'#type' => 'submit',
			'#value' => 'Bandingkan Header'
		);	
		$form['formdata']['bandingkan_rekening'] = array (
			'#type' => 'submit',
			'#value' => 'Bandingkan Rekening'
		);	
		if (arg(7)=='bandingkan_rekening') {
			$form['formdata']['bandingkan_rekeningpdf'] = array (
				'#type' => 'submit',
				'#value' => 'Bandingkan Rekening PDF'
			);	
		} else if (arg(7)=='bandingkan_header'){
			$form['formdata']['bandingkan_headerpdf'] = array (
				'#type' => 'submit',
				'#value' => 'Bandingkan Header PDF'
			);	
		}
	}
...
function kegiatanskpdpra_print_form_submit($form, &$form_state) {
	//$kodeuk = $form_state['values']['kodeuk'];
	$tipedok = $form_state['values']['tipedok'];
	$kodekeg = $form_state['values']['kodekeg'];
	$topmargin = $form_state['values']['topmargin'];
	
	if ($form_state['clicked_button']['#value'] == $form_state['values']['submitsampul']){ 
		$uri = 'apbd/kegiatanskpd_pra/print/' . $kodekeg . '/'. $topmargin . '/' . $tipedok . '/pdf' ;
		$uri .= '/sampul';
		drupal_goto($uri);
	} else if ($form_state['clicked_button']['#value'] == $form_state['values']['bandingkan_header']){
		$uri = 'apbd/kegiatanskpd_pra/print/' . $kodekeg . '/'. $topmargin . '/' . $tipedok . '/pdf' ;
		$uri .= '/bandingkan_header';
		drupal_goto($uri);
	} else if ($form_state['clicked_button']['#value'] == $form_state['values']['bandingkan_rekening']){
		$uri = 'apbd/kegiatanskpd_pra/print/' . $kodekeg . '/'. $topmargin . '/' . $tipedok . '/pdf' ;
		$uri .= '/bandingkan_rekening';
		drupal_goto($uri);
	} else if ($form_state['clicked_button']['#value'] == $form_state['values']['bandingkan_headerpdf']){
		$uri = 'apbd/kegiatanskpd_pra/print/' . $kodekeg . '/'. $topmargin . '/' . $tipedok . '/pdf' ;
		$uri .= '/bandingkan_header/pdf';
		drupal_goto($uri);
	} else if ($form_state['clicked_button']['#value'] == $form_state['values']['bandingkan_rekeningpdf']){
		$uri = 'apbd/kegiatanskpd_pra/print/' . $kodekeg . '/'. $topmargin . '/' . $tipedok . '/pdf' ;
		$uri .= '/bandingkan_rekening/pdf';
		drupal_goto($uri);
	}
...
return '<font color="Red">' . $catatan . '</font>';
}
...
function GenReportFormContentafter($kodekeg) {
...
function GenReportFormHeaderafter($kodekeg, $tipedok) {
	...
	*/
	...
	$rowskegiatan[]= array ( 
							 array('data' => '',  'width'=> '500px','colspan'=> '7px', 'style' => 'font-size:1.3em; text-align:center;'),
							 array('data' => '', 'width' => '20px', 'colspan'=>'3', 'style' => 'font-size:1.3em; text-align:center;'),
		);
	$rowskegiatan[]= array ( 
							 array('data' => 'SETELAH DIKOREKSI',  'width'=> '500px','colspan'=> '7px', 'style' => 'font-size:1.5em; text-align:center;color: blue;'),
							 array('data' => '', 'width' => '20px', 'colspan'=>'3', 'style' => 'font-size:1.3em; text-align:center;'),
		);
	$rowskegiatan[]= array ( 
							 array('data' => '',  'width'=> '500px','colspan'=> '7px', 'style' => 'font-size:1.3em; text-align:center;'),
	...
	*/
	...
	if ($jenis==2) {
		...
		$rowskegiatan[]= array (
							 array('data' => 'Hasil',  'width'=> '175px', 'colspan'=>'2',  'style' => 'border-left: 1px solid black; border-right: 1px solid black; text-align:left;'),
							 array('data' => $hasilsasaran, 'width' => '350px', 'colspan'=>'3',  'style' => 'border-right: 1px solid black; text-align:left;border-bottom: 1px solid black;'),
							 array('data' => $hasiltarget, 'width' => '350', 'colspan'=>'2',  'style' => 'border-right: 1px solid black;border-bottom: 1px solid black; text-align:left;'),
							 );	
	...
	$rowskegiatan[]= array (
						 array('data' => 'Rincian Dokumen Pelaksanaan Anggaran Belanja Tidak Langsung Satuan Kerja Perangkat Daerah',   'width' => '875', 'colspan'=>'7',  'style' => 'border-left: 1px solid black;  border-right: 1px solid black;  border-top: 1px solid black;border-bottom: 1px solid black; text-align:center;'),
						 );							 
