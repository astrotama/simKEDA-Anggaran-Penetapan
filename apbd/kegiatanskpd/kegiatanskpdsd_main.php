...
function GenDataView($kodeuk) {
	...
	$sql = 'select count(kodekeg) jumlahkeg, sum(total) totalsd from kegiatanskpd where inaktif=0 ' . $where;
	$res = db_query($sql);
	if ($res) {
	...
	$sql = 'select sumberdana1, count(kodekeg) jumlahkeg, sum(total) jumlahsd from kegiatanskpd where inaktif=0 ' . $where . ' group by sumberdana1';
