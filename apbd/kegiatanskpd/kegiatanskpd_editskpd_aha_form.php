...
function kegiatanskpd_editskpd_form(&$form_state, $my_values = array()) {
	...
	if ($data = db_fetch_object($pres)) $subbidang = $data->namasuk;
	
	//seksi
	$pquery = sprintf('select namapa from {PelakuAktivitas} where kodepa=\'%s\'', $kodepa);
	$pres = db_query($pquery);
	if ($data = db_fetch_object($pres)) $namapa = $data->namapa;
  ...
  // second dropdown.
	...
	$form['skpd'] = array(
		...
		'#ahah' => array(
		  'path' => 'apbd/kegiatanskpd/editskpd/callback',
...
$kodeuk = apbd_getuseruk();	
$query= sprintf('SELECT  s.namapa , u.namasuk FROM PelakuAktivitas as s inner join subunitkerja as u on s.kodesuk=u.kodesuk where kodeuk=\'%s\'', $kodeuk);
