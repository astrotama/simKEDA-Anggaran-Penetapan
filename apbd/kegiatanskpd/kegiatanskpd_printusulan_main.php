				...
				while ($dataobyek = db_fetch_object($resultobyek)) {
					...
					if ($result) {
						...
						$rowsrek[] = array (
							...
							//DETIL
							$sql = 'select iddetil,uraian,unitjumlah,unitsatuan,volumjumlah,volumsatuan,harga,total, pengelompokan from {anggperkegdetilrevisi} where kodekeg=\'%s\' and kodero=\'%s\' and total>0 order by nourut asc,iddetil';
							$fsql = sprintf($sql, db_escape_string($kodekeg), db_escape_string($data->kodero));
							////drupal_set_message($fsql);
...
function PrintSuratPermohonan($id) {
	...
	$sql = sprintf('select right(k.kodekeg, 3) nomorkeg, k.kodepro, k.kegiatan, 
...
function PrintMatriksPermohonan($id) {
	...
	$pquery = sprintf('select p.kodepro, k.lokasi, k.tw1, k.tw2, k.tw3, k.tw4, k.kegiatan, k.sumberdana1, k.kelompoksasaran, p.kodepro, p.program, k.total, right(k.kodekeg, 3) nomorkeg from {kegiatanrevisi} k inner join {program} p on k.kodepro=p.kodepro where kodekeg=\'%s\'', db_escape_string($kodekeg));
	//drupal_set_message($pquery);
	$pres = db_query($pquery);
	if ($data = db_fetch_object($pres)) {
	...
	$pquery = sprintf('select lokasi, tw1, tw2, tw3, tw4, kegiatan, sumberdana1, kelompoksasaran, p.kodepro, p.program, k.total, right(k.kodekeg, 3) nomorkeg 
...
function PrintMatriksPermohonanL($id) {
	...
	*/
	...
	$pquery = sprintf('select lokasi, tw1, tw2, tw3, tw4, kegiatan, sumberdana1, kelompoksasaran, p.kodepro, p.program, k.total, right(k.kodekeg, 3) nomorkeg   from {kegiatanrevisi} k left join {program} p on k.kodepro=p.kodepro where kodekeg=\'%s\'', db_escape_string($kodekeg));
	$pres = db_query($pquery);
	if ($data = db_fetch_object($pres)) {
		$lokasi_per = str_replace('||',', ', $data->lokasi);
		$sumberdana1_per = $data->sumberdana1;
	...
	$pquery = sprintf('select lokasi, tw1, tw2, tw3, tw4, kegiatan, sumberdana1, kelompoksasaran, p.kodepro, p.program, k.total, right(k.kodekeg, 3) nomorkeg  from {kegiatanskpd} k inner join {program} p on k.kodepro=p.kodepro where kodekeg=\'%s\'', db_escape_string($kodekeg));
	////drupal_set_message($pquery);
	$pres = db_query($pquery);
	if ($data = db_fetch_object($pres)) {
		$lokasi_pen = str_replace('||',', ', $data->lokasi);
