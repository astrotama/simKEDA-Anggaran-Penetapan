...
function GenDataView($kodeuk , $sumberdana , $statusisi , $kodesuk , $statustw , 
	...
	if (($rekening == '') and ($rincian =='')) {
		...
		$sql = "select k.kodekeg,k.nomorkeg,k.tahun,k.kodepro,k.kodeuk,k.kegiatan,k.lokasi, 				
				k.programtarget,k.total,k.plafon,u.namasingkat, k.adminok, k.sumberdana1 sumberdana, k.inaktif,((k.tw1+k.tw2+k.tw3+k.tw4)=k.plafon) twok from {kegiatanskpd} k inner join {unitkerja} u on (k.kodeuk=u.kodeuk) " . $where;
		$fsql = $sql;
		...
		$sql = "select distinct k.kodekeg,k.nomorkeg,k.tahun,k.kodepro,k.kodeuk, k.kegiatan,k.lokasi, 				
				k.programtarget,k.total,k.plafon,u.namasingkat, k.adminok, k.sumberdana1 sumberdana, k.inaktif,((k.tw1+k.tw2+k.tw3+k.tw4)=k.plafon) twok from {kegiatanskpd} k inner join {unitkerja} u on (k.kodeuk=u.kodeuk) inner join {anggperkeg} a on k.kodekeg=a.kodekeg " . $where;
		$fsql = $sql;
		...
		$where = ' where ' . sprintf(' k.tahun=%s ', $tahun) . $qlike ;
		$sql = "select distinct  k.kodekeg,k.nomorkeg,k.tahun,k.kodepro,k.kodeuk,k.kegiatan,k.lokasi, 				
				k.programtarget,k.total,k.plafon,u.namasingkat, k.adminok, k.sumberdana1 sumberdana, k.inaktif,((k.tw1+k.tw2+k.tw3+k.tw4)=k.plafon) twok from {kegiatanskpd} k inner join {unitkerja} u on (k.kodeuk=u.kodeuk) inner join {anggperkeg} a on k.kodekeg=a.kodekeg inner join {anggperkegdetil} d on a.kodekeg=d.kodekeg and a.kodero=d.kodero " . $where;
		$fsql = $sql;
