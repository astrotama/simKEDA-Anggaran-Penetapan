...
function kegiatanskpd1000_main($arg=NULL, $nama=NULL) {
    ...
    $where = ' where ((total mod 1000>0) or (tw1 mod 1000>0) or (tw2 mod 1000>0) or (tw3 mod 1000>0) or (tw4 mod 1000>0)) ' . $customwhere . $qlike ;

	$sql = "select k.kodekeg,k.nomorkeg,k.tahun,k.kodepro,k.kodeuk,k.kegiatan,k.lokasi,k.programtarget,k.total,
			k.plafon,u.namasingkat, k.adminok, k.sumberdana1 sumberdana, k.inaktif, concat_ws(' ', concat(p.kodeu,p.np), u.kodedinas, k.nomorkeg) as koderesmi 
			from {kegiatanskpd} k inner join {unitkerja} u on ( k.kodeuk=u.kodeuk) inner join {program} p on (k.kodepro = p.kodepro) " . $where;
