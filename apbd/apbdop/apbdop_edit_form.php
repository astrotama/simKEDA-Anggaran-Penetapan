...
function apbdop_edit_form(){
	...
	if (strpos($referer, 'apbd/manageuser/edit/')>0)
		$referer = $_SESSION["manageuser_lastpage"];
	else
		$_SESSION["manageuser_lastpage"] = $referer;	
...
//    );
    ...
    $form['formdata']['submit'] = array (
        '#type' => 'submit',
		'#suffix' => "&nbsp;<a href='" . $referer . "' class='btn_blue' style='color: white'>Batal</a>",
        '#value' => 'Simpan'
    );
    return $form;
}
...
function apbdop_edit_form_submit($form, &$form_state) {
    ...
    if ($e_username=='')
    {
		...
		if ($user) {
			...
			$res = db_query(db_rewrite_sql($sql), array($username, $nama, $kodeuk, $kodesuk));
		}
    } else {
        ...
        $res = db_query(db_rewrite_sql($sql), array($nama, $kodeuk, $kodesuk, $e_username));
    }
	if ($user) {
		//$perm = apbd_perm();
