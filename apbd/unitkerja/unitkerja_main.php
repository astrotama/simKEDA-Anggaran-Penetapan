...
function unitkerja_main($arg=NULL, $nama=NULL) {
    ...
    if ($result) {
        while ($data = db_fetch_object($result)) {
			...
			$adapad = false;
			$sqlcek = 'select kodeuk from {anggperuk} where kodeuk=\'%s\'' ;					
			//drupal_set_message($sql . $kodekeg);
			$rescek = db_query(db_rewrite_sql($sqlcek), array ($data->kodeuk));
			if ($rescek) {
			...
			if ($adapad)
				$skpd = l('<strong>' . $data->namauk . '</strong>', 'apbd/unitkerja/edit/' . $data->kodeuk, array('html'=>TRUE));	
			else
