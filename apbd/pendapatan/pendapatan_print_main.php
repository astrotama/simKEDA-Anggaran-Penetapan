...
drupal_add_css('files/css/kegiatancam.css');
	...
	if (isset($exportpdf) && ($exportpdf=='pdf'))  {
		...
		$pquery = "SELECT u.ttd FROM {apbdop} u where u.username='arief' or u.username='kendar'" ;
		$res = db_query($pquery);
		/*
		$sql = 'SELECT u.ttd FROM {apbdop} u where u.username=\'%s\' or u.username=\'%s\'';
		$fsql = sprintf($sql, db_escape_string('arief','kendar'));
		$res = db_query($fsql);
		*/
		$link=array();
		$ind=0; 
