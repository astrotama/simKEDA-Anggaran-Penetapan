...
function kegiatanrevisi_edit4_form_submit($form, &$form_state) {
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['kembali']) {
		...
		if ($id=='0') {
			...
			if (isSuperuser())
				$sql = sprintf("update {kegiatanrevisiperubahan} set jenisrevisi='%s', subjenisrevisi='%s', kodekeg='%s', geserblokir='%s', geserrincian='%s', geserobyek='%s', lokasi='%s', sumberdana='%s', kinerja='%s', sasaran='%s', detiluraian='%s', rab='%s', triwulan='%s', lainnya='%s', alasan1='%s', alasan2='%s', alasan3='%s', nosurat='%s', tglsurat='%s', dokumen='%s' where id='%s'", db_escape_string($jenisrevisi), db_escape_string($subjenisrevisi), db_escape_string($kodekeg), db_escape_string($geserblokir), db_escape_string($geserrincian), db_escape_string($geserobyek), db_escape_string($lokasi), db_escape_string($sumberdana), db_escape_string($kinerja), db_escape_string($sasaran), db_escape_string($detiluraian), db_escape_string($rab), db_escape_string($triwulan), db_escape_string($lainnya), db_escape_string($alasan1), db_escape_string($alasan2), db_escape_string($alasan3), db_escape_string($nosurat), db_escape_string($tglsurat), db_escape_string($dokumen), db_escape_string($id));
			else
		...
		if (($e_kodekeg!='') and ($e_kodekeg != $kodekeg)) {
			...
			//REINSERT
            $query = sprintf("insert into {kegiatanrevisi} select * from {kegiatanskpd} where kodekeg='%s'", db_escape_string($e_kodekeg));
            $res = db_query($query);
			if ($res == false) $emsg .= '4';
			
            $query = sprintf("insert into {anggperkegrevisi} select * from {anggperkeg} where kodekeg='%s'", db_escape_string($e_kodekeg));
            $res = db_query($query);
			if ($res == false) $emsg .= '5';

            $query = sprintf("insert into {anggperkegdetilrevisi} select * from {anggperkegdetil} where kodekeg='%s'", db_escape_string($e_kodekeg));
            $res = db_query($query);
			if ($res == false) $emsg .= '6';		
		}
		*/
