		...
		case '3':
			$form['formdata']['subjenisrevisi']= array(
				...
				'#options' => array(	
					 ...
					 '2' => t('[2] Kegiatan Baru dari DAK/BANPROV' . $labelkegiatan ),
				   ),
			);	
...
function kegiatanrevisi_edit2_form_validate($form, &$form_state) {
	...
	if (($jenisrevisi=='3') and ($subjenisrevisi=='1')) {
		$sql = sprintf('select count(kodekeg) jumlah from {kegiatanperubahan} where kodeuk=\'%s\'', $kodeuk);
		$sql .= " and sumberdana1 in ('DAK', 'BANPROV')";	
		$res = db_query($sql);
		if ($res) {
			if ($data = db_fetch_object($res)) {

			  if ($data->jumlah==0) {
				form_set_error('subjenisrevisi', t('SKPD tidak punya kegiatan dari DAK/BANPROV.'));
			  }
