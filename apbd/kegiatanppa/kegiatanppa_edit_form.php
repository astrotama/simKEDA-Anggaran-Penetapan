...
function kegiatanppa_edit_form() {
    ...
    if (isset($kodeuk)) 
    {
        ...
        $sql = 'select namasingkat, penno, pentgl, penok, btlno, btltgl, btlok, blno, bltgl from {dpanomor2} d inner join {unitkerja} u on d.kodeuk=u.kodeuk where u.kodeuk=\'%s\'';
	...
	$form['submit'] = array (
		'#type' => 'submit',
		'#suffix' => "&nbsp;<a href='/apbd/kegiatanppa' class='btn_blue' style='color: white'>Tutup</a>",
		'#value' => 'Simpan'
	);
...
function kegiatanppa_edit_form_validate($form, &$form_state) {
	...
	if ($nomor !='') {
		...
		$tipe = $form_state['values']['tipe'];
			switch($tipe) {
				case 'pen':
					$sql = 'select penno from {dpanomor2} where kodeuk<>\'%s\' and penno=\'%s\'';
					break;
				case 'btl':
					$sql = 'select btlno from {dpanomor2} where kodeuk<>\'%s\' and btlno=\'%s\'';
					break;
				case 'bl':
					$sql = 'select blno from {dpanomor2} where kodeuk<>\'%s\' and blno=\'%s\'';
					break;
			}
...
function kegiatanppa_edit_form_submit($form, &$form_state) {
	...
	switch($tipe) {
		case 'pen':
			$sql = "update {dpanomor2} set penno='%s', pentgl='%s' where kodeuk='%s'";
			break;
			
		case 'btl':
			$sql = "update {dpanomor2} set btlno='%s', btltgl='%s' where kodeuk='%s'";
			break;
			
		case 'bl':
			$sql = "update {dpanomor2} set blno='%s', bltgl='%s' where kodeuk='%s'";
			break;
	}
...
function getnomorBL() {
	...
	$query = "select max(blno) maxno from {dpanomor2}";
	$pres = db_query($query);
...
function getnomorBTL() {
	...
	$query = "select max(btlno) maxno from {dpanomor2}";
	$pres = db_query($query);
	if ($data=db_fetch_object($pres)) {
		$v = $data->maxno;
		$iv = intval($v);
...
function getnomorPEN() {
	...
	$query = "select max(penno) maxno from {dpanomor2}";
	$pres = db_query($query);
	if ($data=db_fetch_object($pres)) {
		$v = $data->maxno;
		$iv = intval($v);
