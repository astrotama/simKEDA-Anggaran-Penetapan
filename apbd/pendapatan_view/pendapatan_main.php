<?php
function pendapatan_view_main($arg=NULL, $nama=NULL) {
	...
	if ($allowedit==false) {
		...
		if ($res) {
			...
			if ($data) {  		
				...
				//drupal_set_message($data->dispensasipendapatan);
	...
	$header = array (
		...
		array('data' => 'Update', 'field'=> 'lastupdate','valign'=>'top'),
	...
	$sql = "select p.kodeuk,p.tahun,p.kodero,p.uraian,p.jumlah,p.jumlahsebelum,p.jumlahsesudah,
			u.namasingkat,p.ketrekening,p.lastupdate from {anggperuk} p left join {unitkerja} u on (p.kodeuk=u.kodeuk) " . $where;
    ...
    if ($result) {
        while ($data = db_fetch_object($result)) {
			...
			if ($allowedit) {
				...
				//$editlink =l('Rekening', 'apbd/pendapatan_view/subkegiatan/' . $data->kodekeg, array('html'=>TRUE));
				//$editlink =l('Detil', 'apbd/pendapatan_view/detil/' . $data->kodeuk . '/'  . $data->kodero, array('html'=>TRUE))  . '&nbsp;';
				//$editlink =l('Detil', 'apbdpendapatan/' . $data->kodeuk . '/'  . $data->kodero, array('html'=>TRUE))  . '&nbsp;';
				
				//$editlink .=l('Hapus', 'apbd/pendapatan_view/delete/' . $data->kodeuk . '/'  . $data->kodero, array('html'=>TRUE));
			}
            $no++;
	...
	if ($no==0) {
		//apbdpendapatan
		//$linknew = l('Rekening Baru', 'apbd/pendapatan_view/edit/', array('html' =>TRUE));	
		// if ($allowedit)
		// 	$linknew = l('Rekening Baru', 'apbdpendapatan/' . $kodeuk , array('html'=>TRUE));
		// else
	...
	if ($allowedit) {
		//if (user_access('kegiatanskpd tambah')) {
			// $btn .= l('Rekening Baru', 'apbdpendapatan/' . $kodeuk , array ('html' => true, 'attributes'=> array ('class'=>'btn_blue', 'style'=>'color:white;'))) . "&nbsp;";
			// $btn .= "&nbsp;" . l('Import RKA per Rekening', 'importpendapatan/pilihrekening/' . $kodeuk , array ('html' => true, 'attributes'=> array ('class'=>'btn_green', 'style'=>'color:white;'))) . "&nbsp;";
			// $btn .= "&nbsp;" . l('Import RKA per SKPD', 'importpendapatan/importrka/AA/' . $kodeuk , array ('html' => true, 'attributes'=> array ('class'=>'btn_green', 'style'=>'color:white;'))) . "&nbsp;";
		//}
	}
...
//	if (user_access('kegiatanskpd tambah'))
//		$output .= l("<img src='/files/button-add.png' title='Tambah data baru'>", 'apbd/pendapatan_view/edit/' , array('html'=>TRUE)) ;
//	if (user_access('kegiatanskpd pencarian'))		
//        $output .= l("<img src='/files/button-search.png' title='Pencarian data'>", 'apbd/pendapatan_view/find/' , array('html'=>TRUE)) ;
...
function pendapatan_main_form_submit($form, &$form_state) {
	...
	$uri = 'apbd/pendapatan_view/filter/' . $kodeuk  . '/' . $kodek ;
	drupal_goto($uri);
