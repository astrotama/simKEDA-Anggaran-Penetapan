...
function DisplayDesa($arg='normal', $nama='') {
    ...
    $header = array (
		...
		array('data' => 'Kecamatan', 'width' => '250px', 'valign'=>'top'),
		array('data' => '', 'width' => '110px', 'valign'=>'top'),
    );

    $sql = 'select nomor,kecamatan from {kecamatanlt} order by nomor';
    $result = db_query($sql);
    ...
    if ($result) {
        while ($data = db_fetch_object($result)) {
			...
			$no++;
            $rows[] = array (
                array('data' => '<input type="checkbox" id="'.$data->kecamatan.'"/>', 'align' => 'right', 'valign'=>'top'),
