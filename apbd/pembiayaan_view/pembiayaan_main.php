<?php
function pembiayaan_view_main($arg=NULL, $nama=NULL) {
	...
	$header = array (
		...
		array('data' => 'Dasar Hukum','valign'=>'top'),
	...
	$sql = "select tahun,kodero,uraian,jumlah,jumlahsebelum,jumlahsesudah,ketrekening,lastupdate from {anggperda} " . $where;
    ...
    if ($result) {
        while ($data = db_fetch_object($result)) {
			...
			if (user_access('kegiatanrkpd edit')) {
				//$uraian = l($data->uraian, 'apbd/pembiayaan_view/edit/' . $data->kodero , array('attributes' => array('target' => '_blank'), 'html' =>TRUE));
				$uraian = $data->uraian;
			} else {
				$uraian = $data->uraian;
			}
			...
			if ($allowedit) {
				if (user_access('kegiatanrkpd penghapusan')) {
					//$editlink =l('Rekening', 'apbd/pembiayaan_view/subkegiatan/' . $data->kodekeg, array('html'=>TRUE));
					//$editlink =l('Detil', 'apbdpembiayaan/' . $data->kodero, array('html'=>TRUE))  . '&nbsp;';
					//$editlink .=l('Hapus', 'apbd/pembiayaan_view/delete/' . $data->kodero, array('html'=>TRUE));
				}
			}
            $no++;
	...
	$status = 0;
	$record = 0;
	$btn .= l('Cetak', 'apbd/pembiayaan_view/print/' . $kodek, array ('html' => true, 'attributes'=> array ('class'=>'btn_blue', 'style'=>'color:white;')));
...
//	if (user_access('kegiatanrkpd tambah'))
//		$output .= l("<img src='/files/button-add.png' title='Tambah data baru'>", 'apbd/pembiayaan_view/edit/' , array('html'=>TRUE)) ;
//	if (user_access('kegiatanrkpd pencarian'))		
//        $output .= l("<img src='/files/button-search.png' title='Pencarian data'>", 'apbd/pembiayaan_view/find/' , array('html'=>TRUE)) ;
...
function pembiayaan_main_form_submit($form, &$form_state) {
	...
	$uri = 'apbd/pembiayaan_view/filter/' . $kodek ;
	drupal_goto($uri);
