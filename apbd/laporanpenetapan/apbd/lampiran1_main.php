		...
		while ($dataakun = db_fetch_object($resultakun)) {
			...
			if ($resultkel) {
				while ($datakel = db_fetch_object($resultkel)) {
					...
					//JENIS
					if ($kodeuk!='00') {
						$sql = 'select j.kodej,j.uraian,sum(jumlah) jumlahx from {anggperuk} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej where kodeuk=\'%s\' and mid(k.kodero,1,2)=\'%s\' and jumlah>0';
						$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datakel->kodek));
						$fsql .= ' group by j.kodej,j.uraian order by j.kodej';
						
					} else {
						$sql = 'select j.kodej,j.uraian,sum(jumlah) jumlahx from {anggperuk} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej where mid(k.kodero,1,2)=\'%s\' and jumlah>0';
					...
					if ($result) {
						while ($data = db_fetch_object($result)) {
							...
							if ($tingkat>=4) {
								if ($kodeuk!='00') {
									$sql = 'select r.kodeo,r.uraian,sum(jumlah) jumlahx from {anggperuk} k inner join {obyek} r on mid(k.kodero,1,5)=r.kodeo where kodeuk=\'%s\' and mid(k.kodero,1,3)=\'%s\' and jumlah>0';
									$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($data->kodej));
									$fsql .= ' group by r.kodeo,r.uraian order by r.kodeo';
									
								} else {
									$sql = 'select r.kodeo,r.uraian,sum(jumlah) jumlahx from {anggperuk} k inner join {obyek} r on mid(k.kodero,1,5)=r.kodeo where mid(k.kodero,1,3)=\'%s\' and jumlah>0';
								...
								if ($resulto) {
									while ($datao = db_fetch_object($resulto)) {
										...
										if ($tingkat>=5) {
											if ($kodeuk!='00') {
												$sql = 'select r.kodero,r.uraian,jumlah jumlahx from {anggperuk} k inner join {rincianobyek} r on k.kodero=r.kodero where kodeuk=\'%s\' and mid(k.kodero,1,5)=\'%s\' and jumlah>0';
												$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datao->kodeo));
												$fsql .= ' order by r.kodero';
												
											} else {
												$sql = 'select r.kodero,r.uraian,sum(jumlah) jumlahx from {anggperuk} k inner join {rincianobyek} r on k.kodero=r.kodero where mid(k.kodero,1,5)=\'%s\' and jumlah>0';
...
function GenReportFormContent($kodeuk, $tingkat, $exportpdf) {
	...
	if ($kodeuk=='00') {
		...
		$rowsrek[] = array (
							 array('data' => '6',  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
							 array('data' => 'PEMBIAYAAN',  'width' => '375px', 'style' => ' border-right: 1px solid black; text-align:left;font-weight:bold;font-size:125%;'),
							 array('data' => '',  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;'),
							 );
	...
	while ($dataakun = db_fetch_object($resultakun)) {
		...
		if ($resultkel) {
			while ($datakel = db_fetch_object($resultkel)) {
				...
				//JENIS
				if ($kodeuk!='00') {
					$sql = 'select j.kodej,j.uraian,sum(jumlah) jumlahx from {anggperuk} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej where k.jumlah>0 and kodeuk=\'%s\' and mid(k.kodero,1,2)=\'%s\'';
					$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datakel->kodek));
					$fsql .= ' group by j.kodej,j.uraian order by j.kodej';
					
				} else {
					$sql = 'select j.kodej,j.uraian,sum(jumlah) jumlahx from {anggperuk} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej where k.jumlah>0 and mid(k.kodero,1,2)=\'%s\'';
					$fsql = sprintf($sql, db_escape_string($datakel->kodek));
					$fsql .= ' group by j.kodej,j.uraian order by j.kodej';
				}
...
function lampiran1_form_submit($form, &$form_state) {
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['tampilkan']) 
		...
		$uri = 'apbd/laporanpenetapan/apbd/lampiran1/' . $kodeuk . '/' . $tingkat . '/'. $topmargin . '/' . $hal1 . '/excel' ;
	else
