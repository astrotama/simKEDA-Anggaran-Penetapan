<?php
function lampiran1_bintang_main() {
...
drupal_add_css('files/css/kegiatancam.css');
	...
	if (isset($exportpdf))  {
		...
		if ($exportpdf=='pdf') {
			...
			$htmlContent = GenReportFormContent($kodeuk, $tingkat, $exportpdf);
...
function GenReportForm($print=0) {
	...
	if ($kodeuk  == '00') {	
		$rowslampiran[]= array (
							 ...
							 array('data' => 'LAMPIRAN I', 'width' => '55px', 'style' => 'border:none; text-align:right;font-size: 75%;'),
							 array('data' => ': PERATURAN ' . $strlamp . ' JEPARA', 'width' => '220px', 'colspan'=>'2',  'style' => 'border:none;text-align:left;font-size: 75%;'),
							 );
		$rowslampiran[]= array (
							 array('data' => '',  'width'=> '290px',  'style' => 'border:none; text-align:left;'),
	...
	$rowsjudul[] = array (array ('data'=>'RINGKASAN ANGGARAN PENDAPATAN DAN BELANJA DAERAH', 'width'=>'520px', 'colspan'=>'3', 'style' =>'border:none; font-weight:900; font-size:1.3em; text-align:center;'));
	$rowsjudul[] = array (array ('data'=> $juduluk, 'width'=>'520px', 'colspan'=>'3', 'style' =>'border:none; text-align:center;'));
	$rowsjudul[] = array (array ('data'=> 'TAHUN ANGGARAN ' . $tahun, 'width'=>'520px', 'colspan'=>'3', 'style' =>'border:none; text-align:center;'));
	...
	$headersrek[] = array (
						 ...
						 array('data' => 'Kode',  'width'=> '55px', 'style' => 'border-bottom: 1px solid black; border-right: 1px solid black; border-top: 2px solid black; text-align:center;'),
	...
	if ($resultakun) {
		while ($dataakun = db_fetch_object($resultakun)) {
			...
			$rowsrek[] = array (
								 ...
								 array('data' => '',  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;font-weight:bold;'),
								 );
				...
				while ($datakel = db_fetch_object($resultkel)) {
					...
					if ($result) {
						while ($data = db_fetch_object($result)) {
							...
							$uraianj = l($uraianj, 'apbd/laporanpenetapan/apbd/lampiran1detil/pendapatan_jenis_uk/' . $data->kodej, array('html'=>TRUE));
							...
							if ($tingkat>=4) {
								...
								if ($resulto) {
									while ($datao = db_fetch_object($resulto)) {
										...
										$uraiano = l($datao->uraian, 'apbd/laporanpenetapan/apbd/lampiran1detil/pendapatan_obyek_uk/' . $datao->kodeo, array('html'=>TRUE));
										...
										$rowsrek[] = array (
															 ...
															 array('data' => $uraiano,  'width' => '375px', 'style' => ' border-right: 1px solid black; text-align:left;'),
										...
										if ($tingkat>=5) {
											...
											if ($resultro) {
												while ($dataro = db_fetch_object($resultro)) {
													...
													$uraian = l($dataro->uraian, 'apbd/laporanpenetapan/apbd/lampiran1detil/pendapatan_rincian_uk/' . $dataro->kodero, array('html'=>TRUE));
													...
													$rowsrek[] = array (
																		 ...
																		 array('data' => $uraian,  'width' => '375px', 'style' => ' border-right: 1px solid black; text-align:left;font-style: italic;'),
		...
		$rowsrek[] = array (
							 ...
							 array('data' => apbd_fn($totalp),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 );
				...
				while ($datakel = db_fetch_object($resultkel)) {
					...
					if ($result) {
						while ($data = db_fetch_object($result)) {
							...
							$uraianj = l($uraianj, 'apbd/laporanpenetapan/apbd/lampiran1detil/belanja_jenis_uk/' . $data->kodej, array('html'=>TRUE));
							...
							if ($tingkat>=4) {
								...
								if ($resulto) {
									while ($datao = db_fetch_object($resulto)) {
										...
										$uraiano = l($datao->uraian, 'apbd/laporanpenetapan/apbd/lampiran1detil/belanja_obyek_uk/' . $datao->kodeo, array('html'=>TRUE));
										...
										if ($tingkat>=5) {
											...
											if ($resultro) {
												while ($dataro = db_fetch_object($resultro)) {
													...
													$uraian_ro = l($dataro->uraian, 'apbd/laporanpenetapan/apbd/lampiran1detil/belanja_rincian_uk/' . $dataro->kodero, array('html'=>TRUE));
												
													$rowsrek[] = array (
																		 array('data' => apbd_format_rek_rincianobyek($dataro->kodero),  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
																		 array('data' => $uraian_ro,  'width' => '375px', 'style' => ' border-right: 1px solid black; text-align:left;font-style: italic;'),
																		 array('data' => apbd_fn($dataro->jumlahx),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;font-style: italic;'),
																		 );	
	...
	$rowsrek[] = array (
						 ...
						 array('data' => apbd_fn($totalp-$totalb),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
						 );
	...
	if ($kodeuk=='00') {
		...
		$rowsrek[] = array (
							 ...
							 array('data' => apbd_fn($totalpm-$totalpk),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 );

		$rowsrek[] = array (
							 array('data' => '',  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; '),
							 array('data' => 'SISA LEBIH ANGGARAN TAHUN BERKENAAN',  'width' => '375px', 'style' => ' border-right: 1px solid black; font-weight:bold;'),
							 array('data' => apbd_fn($totalp-$totalb+$totalpm-$totalpk),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 );
	...
	$rowsrek[] = array (
						 ...
						 array('data' => '',  'width' => '90px', 'style' => ' border-right: 1px solid black;'),
						 );
...
function GenReportFormHeader($tingkat) {
	...
	if ($kodeuk  == '00') {	
		$rowslampiran[]= array (
							 array('data' => '',  'width'=> '270px', 'style' => 'border:none; text-align:left;'),
	...
	$rowsjudul[] = array (array ('data'=> $juduluk, 'width'=>'520px', 'colspan'=>'3', 'style' =>'border:none; text-align:center;'));
	$rowsjudul[] = array (array ('data'=> 'TAHUN ANGGARAN ' . $tahun, 'width'=>'520px', 'colspan'=>'3', 'style' =>'border:none; text-align:center;'));
	//$rowsjudul[] = array (array ('data'=> '', 'width'=>'520px', 'colspan'=>'3', 'style' =>'border:none; text-align:center;'));
...
function GenReportFormContent($kodeuk, $tingkat, $exportpdf) {
	//drupal_set_message($tingkat);
	$headersrek[] = array (
						 ...
						 array('data' => 'Kode',  'width'=> '55px', 'style' => 'border-left: 1px solid black; border-bottom: 1px solid black; border-right: 1px solid black; border-top: 2px solid black; text-align:center;'),
						 array('data' => 'Uraian',  'width' => '205x', 'style' => 'border-bottom: 1px solid black; border-right: 1px solid black; border-top: 2px solid black; text-align:center;'),
						 array('data' => 'Jumlah',  'width' => '90px', 'style' => 'border-bottom: 1px solid black; border-right: 1px solid black; border-top: 2px solid black; text-align:center;'),
	...
	if ($resultakun) {
		while ($dataakun = db_fetch_object($resultakun)) {
			...
			$rowsrek[] = array (
								 array('data' => $dataakun->kodea,  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
								 array('data' => $dataakun->uraian,  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:left;font-weight:bold;'),
								 array('data' => '',  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;font-weight:bold;'),
								 array('data' => '',  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;font-weight:bold;'),
								 array('data' => '',  'width' => '80px', 'style' => ' border-right: 1px solid black; text-align:right;font-weight:bold;'),
								 );
			...
			if ($resultkel) {
				while ($datakel = db_fetch_object($resultkel)) {
					$rowsrek[] = array (
										 array('data' => $datakel->kodek,  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
										 array('data' => $datakel->uraian,  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:left;font-weight:bold;'),
										 array('data' => apbd_fn($datakel->jumlahx),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-bottom: 1px solid black; text-align:right;font-weight:bold;'),
										 array('data' => apbd_fn($datakel->jumlahx),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-bottom: 1px solid black; text-align:right;font-weight:bold;'),
										 array('data' => apbd_fn(0),  'width' => '80px', 'style' => ' border-right: 1px solid black; border-bottom: 1px solid black; text-align:right;font-weight:bold;'),
										 );		
					...
					if ($result) {
						while ($data = db_fetch_object($result)) {
							...
							$rowsrek[] = array (
												 array('data' => ($data->kodej),  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
												 array('data' => $uraianj,  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:left;'),
												 array('data' => apbd_fn($data->jumlahx),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;' . $bold),
												 array('data' => apbd_fn($data->jumlahx),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;' . $bold),
												 array('data' => apbd_fn(0),  'width' => '80px', 'style' => ' border-right: 1px solid black; text-align:right;' . $bold),
												 );
							...
							if ($tingkat>=4) {
								...
								if ($resulto) {
									while ($datao = db_fetch_object($resulto)) {
										$rowsrek[] = array (
															 array('data' => apbd_format_rek_obyek($datao->kodeo),  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
															 array('data' => $datao->uraian,  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:left;'),
															 array('data' => apbd_fn($datao->jumlahx),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;'),
															 array('data' => apbd_fn($datao->jumlahx),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;'),
															 array('data' => apbd_fn(0),  'width' => '80px', 'style' => ' border-right: 1px solid black; text-align:right;'),
															 );	
										...
										if ($tingkat>=5) {
											...
											if ($resultro) {
												while ($dataro = db_fetch_object($resultro)) {
													$rowsrek[] = array (
																		 array('data' => apbd_format_rek_rincianobyek($dataro->kodero),  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
																		 array('data' => $dataro->uraian,  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:left;font-style: italic;'),
																		 array('data' => apbd_fn($dataro->jumlahx),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;font-style: italic;'),
																		 array('data' => apbd_fn($dataro->jumlahx),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;font-style: italic;'),
																		 array('data' => apbd_fn(0),  'width' => '80px', 'style' => ' border-right: 1px solid black; text-align:right;font-style: italic;'),
																		 );	
		...
		$rowsrek[] = array (
							 array('data' => '',  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; '),
							 array('data' => 'JUMLAH PENDAPATAN',  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:right; font-weight:bold;'),
							 array('data' => apbd_fn($totalp),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 array('data' => apbd_fn($totalp),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 array('data' => apbd_fn(0),  'width' => '80px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 );
	...
	if ($kodeuk!='00') {
		$where = ' and g.kodeuk=\'%s\'';
		$sql = 'select a.kodea,a.uraian,sum(k.jumlah) jumlahx,sum(k.anggaran) anggaranx from {anggperkeg} k  inner join {anggaran} a on mid(k.kodero,1,1)=a.kodea inner join {kegiatanskpd} g on k.kodekeg=g.kodekeg where g.inaktif=0 ' . $where;
		$fsql = sprintf($sql, db_escape_string($kodeuk));
		$fsql .= ' group by a.kodea,a.uraian order by a.kodea';
	
	} else {
		$sql = 'select a.kodea,a.uraian,sum(k.jumlah) jumlahx,sum(k.anggaran) anggaranx from {anggperkeg} k  inner join {anggaran} a on mid(k.kodero,1,1)=a.kodea inner join {kegiatanskpd} g on k.kodekeg=g.kodekeg ';
		$fsql = $sql . ' where g.inaktif=0 group by a.kodea,a.uraian order by a.kodea';
	...
	if ($resultakun) {
		while ($dataakun = db_fetch_object($resultakun)) {
			$totalb += $dataakun->jumlahx;
			$totalb_bintang += $dataakun->anggaranx;
			$rowsrek[] = array (
								 array('data' => $dataakun->kodea,  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
								 array('data' => $dataakun->uraian,  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:left;font-weight:bold;'),
								 array('data' => '',  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;'),
								 array('data' => '',  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;'),
								 array('data' => '',  'width' => '80px', 'style' => ' border-right: 1px solid black; text-align:right;'),
								 );
				
			//KELOMPOK
			if ($kodeuk!='00') {
				$sql = 'select x.kodek,x.uraian,sum(k.jumlah) jumlahx,sum(k.anggaran) anggaranx from {anggperkeg} k inner join {kelompok} x on mid(k.kodero,1,2)=x.kodek inner join {kegiatanskpd} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and mid(k.kodero,1,1)=\'%s\'';
				$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($dataakun->kodea));
				$fsql .= ' group by x.kodek,x.uraian order by x.kodek';
			
			} else {
				$sql = 'select x.kodek,x.uraian,sum(k.jumlah) jumlahx,sum(k.anggaran) anggaranx from {anggperkeg} k inner join {kelompok} x on mid(k.kodero,1,2)=x.kodek inner join {kegiatanskpd} g on k.kodekeg=g.kodekeg where g.inaktif=0 and mid(k.kodero,1,1)=\'%s\'';
				$fsql = sprintf($sql, db_escape_string($dataakun->kodea));
				$fsql .= ' group by x.kodek,x.uraian order by  x.kodek';
			}
			//drupal_set_message( $fsql);
			...
			if ($resultkel) {
				while ($datakel = db_fetch_object($resultkel)) {
					$rowsrek[] = array (
										 ...
										 array('data' => $datakel->uraian,  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:left;font-weight:bold;'),
										 array('data' => apbd_fn($datakel->jumlahx),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-bottom: 1px solid black; text-align:right;font-weight:bold;'),
										 array('data' => apbd_fn($datakel->anggaranx),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-bottom: 1px solid black; text-align:right;font-weight:bold;'),
										 array('data' => apbd_fn($datakel->jumlahx-$datakel->anggaranx),  'width' => '80px', 'style' => ' border-right: 1px solid black; border-bottom: 1px solid black; text-align:right;font-weight:bold;'),
										 );		


					//JENIS
					if ($kodeuk!='00') { 
						$sql = 'select j.kodej,j.uraian,sum(k.jumlah) jumlahx,sum(k.anggaran) anggaranx from {anggperkeg} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej  inner join {kegiatanskpd} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and mid(k.kodero,1,2)=\'%s\'';
						$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datakel->kodek));
						$fsql .= ' group by j.kodej,j.uraian order by j.kodej';
					
					} else {
						$sql = 'select j.kodej,j.uraian,sum(k.jumlah) jumlahx,sum(k.anggaran) anggaranx from {anggperkeg} k inner join {jenis} j on mid(k.kodero,1,3)=j.kodej  inner join {kegiatanskpd} g on k.kodekeg=g.kodekeg where g.inaktif=0 and mid(k.kodero,1,2)=\'%s\'';
						$fsql = sprintf($sql, db_escape_string($datakel->kodek));
						$fsql .= ' group by j.kodej,j.uraian order by mid(k.kodero,1,3)';
					}
					//drupal_set_message( $fsql);
					...
					if ($result) {
						while ($data = db_fetch_object($result)) {
							...
							$rowsrek[] = array (
												 ...
												 array('data' => $uraianj,  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:left;'),
												 array('data' => apbd_fn($data->jumlahx),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;' . $bold),
												 array('data' => apbd_fn($data->anggaranx),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;' . $bold),
												 array('data' => apbd_fn($data->jumlahx-$data->anggaranx),  'width' => '80px', 'style' => ' border-right: 1px solid black; text-align:right;' . $bold),
												 );
							
							//OBYEK
							if ($tingkat>=4) {
								if ($kodeuk!='00') { 
									$sql = 'select r.kodeo,r.uraian,sum(k.jumlah) jumlahx,sum(k.anggaran) anggaranx from {anggperkeg} k inner join {obyek} r on mid(k.kodero,1,5)=r.kodeo inner join {kegiatanskpd} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and mid(k.kodero,1,3)=\'%s\'';
									$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($data->kodej));
									$fsql .= ' group by r.kodeo,r.uraian order by r.kodeo';
								
								} else {
									$sql = 'select r.kodeo,r.uraian,sum(k.jumlah) jumlahx,sum(k.anggaran) anggaranx from {anggperkeg} k inner join {obyek} r on mid(k.kodero,1,5)=r.kodeo inner join {kegiatanskpd} g on k.kodekeg=g.kodekeg where g.inaktif=0 and mid(k.kodero,1,3)=\'%s\'';
									$fsql = sprintf($sql, db_escape_string($data->kodej));
									$fsql .= ' group by r.kodeo,r.uraian order by r.kodeo';
								}
								$resulto = db_query($fsql);
								if ($resulto) {
									while ($datao = db_fetch_object($resulto)) {
										$rowsrek[] = array (
															 ...
															 array('data' => $datao->uraian,  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:left;'),
															 array('data' => apbd_fn($datao->jumlahx),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;'),
															 array('data' => apbd_fn($datao->anggaranx),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;'),
															 array('data' => apbd_fn($datao->jumlahx-$datao->anggaranx),  'width' => '80px', 'style' => ' border-right: 1px solid black; text-align:right;'),
															 );	

										//RINCIAN OBYEK 
										if ($tingkat>=5) {
											if ($kodeuk!='00') { 
												$sql = 'select r.kodero,r.uraian,sum(k.jumlah) jumlahx,sum(k.anggaran) anggaranx from {anggperkeg} k inner join {rincianobyek} r on k.kodero=r.kodero inner join {kegiatanskpd} g on k.kodekeg=g.kodekeg where g.inaktif=0 and g.kodeuk=\'%s\' and mid(k.kodero,1,5)=\'%s\'';
												$fsql = sprintf($sql, db_escape_string($kodeuk), db_escape_string($datao->kodeo));
												$fsql .= ' group by r.kodero,r.uraian order by r.kodero';
											
											} else {
												$sql = 'select r.kodero,r.uraian,sum(k.jumlah) jumlahx,sum(k.anggaran) anggaranx from {anggperkeg} k inner join {rincianobyek} r on k.kodero=r.kodero inner join {kegiatanskpd} g on k.kodekeg=g.kodekeg where g.inaktif=0 and mid(k.kodero,1,5)=\'%s\'';
												$fsql = sprintf($sql, db_escape_string($datao->kodeo));
												$fsql .= ' group by r.kodero,r.uraian order by r.kodero';
											} 
											//drupal_set_message($fsql);
											...
											if ($resultro) {
												while ($dataro = db_fetch_object($resultro)) {
													...
													if ($dataro->kodero=='52219003')
														$rowsrek[] = array (
																			 array('data' => apbd_format_rek_rincianobyek($dataro->kodero),  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
																			 array('data' => $dataro->uraian,  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:left;font-style: italic;font-size:small;'),
																			 array('data' => apbd_fn($dataro->jumlahx),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;font-style: italic;'),
																			 array('data' => apbd_fn($dataro->anggaranx),  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;font-style: italic;'),
																			 array('data' => apbd_fn($dataro->jumlahx-$dataro->anggaranx),  'width' => '80px', 'style' => ' border-right: 1px solid black; text-align:right;font-style: italic;'),
																			 );	
													else 
													$rowsrek[] = array (
																		 array('data' => apbd_format_rek_rincianobyek($dataro->kodero),  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
		...
		$rowsrek[] = array (
							 array('data' => '',  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; '),
							 array('data' => 'JUMLAH BELANJA',  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:right; font-weight:bold;'),
							 array('data' => apbd_fn($totalb),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 array('data' => apbd_fn($totalb_bintang),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 array('data' => apbd_fn($totalb-$totalb_bintang),  'width' => '80px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 );
	}
	$rowsrek[] = array (
						 array('data' => '',  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; '),
						 array('data' => 'SURPLUS / DEFISIT',  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:right; font-weight:bold;'),
						 array('data' => apbd_fn($totalp-$totalb),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
						 array('data' => apbd_fn($totalp-$totalb_bintang),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
						 array('data' => apbd_fn(($totalp-$totalb) - ($totalp-$totalb_bintang)),  'width' => '80px', 'style' => ' border-right: 1px solid black; border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
						 );
	...
	if ($kodeuk=='00') {
		...
		$rowsrek[] = array (
							 array('data' => '6',  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; text-align:left;'),
							 array('data' => 'PEMBIAYAAN',  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:left;font-weight:bold;'),
							 array('data' => '',  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;'),
							 array('data' => '',  'width' => '90px', 'style' => ' border-right: 1px solid black; text-align:right;'),
							 array('data' => '',  'width' => '80px', 'style' => ' border-right: 1px solid black; text-align:right;'),
							 );
		...
		$rowsrek[] = array (
							 array('data' => '',  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; '),
							 array('data' => 'PEMBIAYAAN NETTO',  'width' => '205px', 'style' => ' border-right: 1px solid black; text-align:right; font-weight:bold;'),
							 array('data' => apbd_fn($totalpm-$totalpk),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 array('data' => apbd_fn($totalpm-$totalpk),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 array('data' => apbd_fn(0),  'width' => '80px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 );
		...
		$rowsrek[] = array (
							 array('data' => '',  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black;'),
							 array('data' => 'SISA LEBIH ANGGARAN TAHUN BERKENAAN',  'width' => '205px', 'style' => ' border-right: 1px solid black; font-weight:bold;'),
							 array('data' => apbd_fn($totalp-$totalb+$totalpm-$totalpk),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 array('data' => apbd_fn($totalp-$totalb_bintang+$totalpm-$totalpk),  'width' => '90px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 array('data' => apbd_fn(($totalp-$totalb+$totalpm-$totalpk) - ($totalp-$totalb_bintang+$totalpm-$totalpk)),  'width' => '80px', 'style' => ' border-right: 1px solid black; border-top: 1px solid black;border-bottom: 2px solid black; text-align:right;font-weight:bold;'),
							 );
	...
	$rowsrek[] = array (
						 array('data' => '',  'width'=> '55px', 'style' => 'border-left: 1px solid black;  border-right: 1px solid black; border-bottom: 1px solid black; '),
						 array('data' => '',  'width' => '205px', 'style' => ' border-right: 1px solid black; border-bottom: 1px solid black; '),
						 array('data' => '',  'width' => '90px', 'style' => ' border-right: 1px solid black; border-bottom: 1px solid black; '),
						 array('data' => '',  'width' => '90px', 'style' => ' border-right: 1px solid black; border-bottom: 1px solid black; '),
						 array('data' => '',  'width' => '80px', 'style' => ' border-right: 1px solid black; border-bottom: 1px solid black; '),
						 );
...
function GenReportFormContentExcel($kodeuk, $tingkat) {
...
if ($resultakun) {
	while ($dataakun = db_fetch_object($resultakun)) {
		...
		$row++;
		$objPHPExcel->setActiveSheetIndex(0)
					->setCellValue('A' . $row, $dataakun->kodea)
					->setCellValue('B' . $row, $dataakun->uraian)
					->setCellValue('C' . $row, $dataakun->jumlahx);
		...
		if ($resultkel) {
			while ($datakel = db_fetch_object($resultkel)) {
				...
				$row++;
				$objPHPExcel->setActiveSheetIndex(0)
							->setCellValue('A' . $row, $datakel->kodek)
							->setCellValue('B' . $row, $datakel->uraian)
							->setCellValue('C' . $row, $datakel->jumlahx);
				...
				if ($result) {
					while ($data = db_fetch_object($result)) {
						...
						$row++;
						$objPHPExcel->setActiveSheetIndex(0)
									->setCellValue('A' . $row, $data->kodej)
									->setCellValue('B' . $row, $data->uraian)
									->setCellValue('C' . $row, $data->jumlahx);
						...
						if ($tingkat>=4) {
							...
							if ($resulto) {
								while ($datao = db_fetch_object($resulto)) {
									...
									$row++;
									$objPHPExcel->setActiveSheetIndex(0)
												->setCellValue('A' . $row, $datao->kodeo)
												->setCellValue('B' . $row, $datao->uraian)
												->setCellValue('C' . $row, $datao->jumlahx);
									...
									if ($tingkat>=5) {
										...
										if ($resultro) {
											while ($dataro = db_fetch_object($resultro)) {
												...
												$row++;
												$objPHPExcel->setActiveSheetIndex(0)
															->setCellValue('A' . $row, $dataro->kodero)
															->setCellValue('B' . $row, $dataro->uraian)
															->setCellValue('C' . $row, $dataro->jumlahx);
...
$objPHPExcel->setActiveSheetIndex(0)
			->setCellValue('A' . $row, '')
			->setCellValue('B' . $row, 'SURPLUS / DEFISIT')
			->setCellValue('C' . $row, $totalp-$totalb);
...
if ($kodeuk=='00') {
	...
	$row++;
	$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A' . $row, '6')
				->setCellValue('B' . $row, 'PEMBIAYAAN')
				->setCellValue('C' . $row, '');
	...
	$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A' . $row, '')
				->setCellValue('B' . $row, 'PEMBIAYAAN NETTO')
				->setCellValue('C' . $row, $totalpm-$totalpk);

	$row++;
	$objPHPExcel->setActiveSheetIndex(0)
				->setCellValue('A' . $row, '')
				->setCellValue('B' . $row, 'SISA LEBIH ANGGARAN TAHUN BERKENAAN')
				->setCellValue('C' . $row, $totalp-$totalb+$totalpm-$totalpk);

						
}

// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('LAMPIRAN I APBD');
...
function lampiran1_bintang_form () {
	$form['formdata'] = array (
		'#type' => 'fieldset',
...
function lampiran1_bintang_form_submit($form, &$form_state) {
	...
	if($form_state['clicked_button']['#value'] == $form_state['values']['tampilkan']) 
        $uri = 'apbd/laporanpenetapan/apbd/lampiran1bintang/' . $kodeuk . '/' . $tingkat . '/'. $topmargin . '/' . $hal1;
	else if($form_state['clicked_button']['#value'] == $form_state['values']['excel']) 
		$uri = 'apbd/laporanpenetapan/apbd/lampiran1bintang/' . $kodeuk . '/' . $tingkat . '/'. $topmargin . '/' . $hal1 . '/excel' ;
	else
		$uri = 'apbd/laporanpenetapan/apbd/lampiran1bintang/' . $kodeuk . '/' . $tingkat . '/'. $topmargin . '/' . $hal1 . '/pdf' ;
