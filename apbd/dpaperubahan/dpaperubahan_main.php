...
function dpaperubahan_main($arg=NULL, $nama=NULL) {
	...
	$sql = "select k.kodekeg,k.nomorkeg,k.tahun,k.kodepro,k.kodeuk,k.kegiatan,k.jenis,
			k.total,k.totalp,u.namasingkat, k.adminok,  k.tw1p, k.tw2p, k.tw3p, k.tw4p, dn.btltgl, dn.bltgl, concat_ws(' ', concat(p.kodeu,p.np), u.kodedinas, k.nomorkeg) as koderesmi, k.r_admin, k.r_geser, k.r_transfer from {kegiatanperubahan" . $strrevisi . "} k inner join {unitkerja} u on ( k.kodeuk=u.kodeuk) inner join {program} p on (k.kodepro = p.kodepro) inner join {dpanomor2} dn on u.kodeuk=dn.kodeuk " . $where;
    ...
    if ($result) {
        while ($data = db_fetch_object($result)) {
			...
			*/
			...
			if ($data->totalp == ($data->tw1p+$data->tw2p+$data->tw3p+$data->tw4p)) {
				...
				*/
				...
				if ((($data->tw1p % $seribu)>0) or (($data->tw2p % $seribu)>0) or (($data->tw3p % $seribu)>0) or (($data->tw4p % $seribu)>0)) {
					$editlink = l('TW', 'apbd/dpaperubahan/triwulan/' . $data->kodekeg, array('html'=>TRUE)) . "&nbsp;";
					$editlink .= 'Cetak';
					...
					//if (isSuperuser())
						$editlink .= l('Cetak', 'apbd/kegiatanskpd/printperubahan/' . $data->kodekeg . '/10/dpa/'. $revisi , array('html'=>TRUE)) ;
					//else
					//	$editlink .= 'Cetak';
				}
				...
				$editlink = l('TW', 'apbd/dpaperubahan/triwulan/' . $data->kodekeg . '/dpa', array('html'=>TRUE)) . "&nbsp;";
				$editlink .= 'Cetak';
			}
            ...
            $no++;
			$editlink .= "&nbsp;" . l('Sampul', 'apbd/kegiatanskpd/printperubahan/' . $data->kodekeg.'/10/dpa/pdf/sampuldppa', array('html'=>TRUE)) ;;
			if (isSuperuser()) { 
	...
	$btn = l('Sampul Depan', 'apbd/kegiatanskpd/printperubahan/' . $kodeuk . '/10/dpa/pdf/sampuld', array ('html' => true, 'attributes'=> array ('class'=>'btn_blue', 'style'=>'color:white;')));
