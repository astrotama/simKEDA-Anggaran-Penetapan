<?php
function apbdops_main($arg=NULL, $nama=NULL) {
	...
	if (isSuperuser()) {
		...
		$sql = 'select u1.username, u1.nama, u1.kodeuk, u2.namasingkat skpd from {apbdop} u1 left join {unitkerja} u2 on (u1.kodeuk=u2.kodeuk) ' . $where;
		...
		$sql = 'select u1.username, u1.nama, u1.kodesuk, u2.namasuk skpd from {apbdop} u1 inner join {subunitkerja} u2 on (u1.kodesuk=u2.kodesuk) ' . $where;
		$fsql = sprintf($sql, addslashes($nama));
	}
    ...
    //$output .= drupal_get_form('apbdops_main_form');
